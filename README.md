...
- **`RATE_LIMITED`** — Yahoo returned a 429 or yfinance's own `YFRateLimitError`. yfinance already retries once internally (swapping cookie strategy) before ever raising this to us, so AlphaLab adds only one further retry with backoff on top — piling on more would just add to the throttling. **Action:** wait and retry later, or reduce how many tickers are requested per run.
- **`NETWORK_UNAVAILABLE`** — a connection-level failure (DNS resolution, timeout, connection reset) reaching Yahoo. yfinance does not retry these internally by default, so AlphaLab retries up to twice with exponential backoff before surfacing this. **Action:** check network/DNS connectivity, then retry.
...
## Canonical research object (`alpha_lab.research`)

`alpha_lab.research.build_stock_research(record)` converts an existing `LiveResearchRecord` into an auditable `StockResearch` object (`alpha_lab.research.model`) without recomputing any score, percentile, or coverage value — those stay owned by `alpha_lab.ratings`/`alpha_lab.screener`. It only regroups already-computed evidence:

- Per-category `CategoryResult` (`score`, `coverage`, `status`, `metrics`, `evidence`, `unavailable_metrics`, `sources`) for exactly the eight documented categories (Business Quality, Earnings Growth, Financial Strength, Momentum, Valuation, Shareholder Return, Analyst Revisions, AI Research), always in that order.
- Per-metric `MetricEvidence` carrying an explicit `MetricStatus` (`AVAILABLE`, `UNAVAILABLE`, `NOT_APPLICABLE`, `INVALID`) plus unit, period, source, retrieval time, and — where the calculation is documented in `alpha_lab.research.formulas` — its formula and any inputs already present among the live raw metrics. `NOT_APPLICABLE`/`INVALID` are defined for future security-type-aware applicability and pre-scoring validation respectively; nothing is classified into them yet.
- A deterministic, documented `confidence` (0–10), separate from `overall_score` and `overall_coverage`: it weights live coverage, how many of the eight categories have any evidence, evidence freshness, and the fraction of available metrics from a recognized primary provider, then applies a flat penalty when the live data-quality check found an issue (stale/missing price, insufficient history, liquidity rule). See `alpha_lab.research.build._confidence` for the exact weights.
- Deterministic `strengths`/`weaknesses` (categories scoring ≥70/≤30 on the existing 0-100 scale) and `risks` (a category with zero evidence, or a data-quality issue) — never derived from missing evidence being treated as a negative. A category with *some* evidence but not enough for a score is `PARTIAL`, not `UNAVAILABLE`; only zero evidence is `UNAVAILABLE`.
- `confidence_breakdown` exposes the same coverage/category-breadth/freshness/source-quality factors `confidence` is computed from (50/20/20/10% weights, plus a data-quality penalty flag) — a structural view of one calculation, not a second one.
- `catalysts` is always `[]` in this stage: it is reserved for AI Research (see below) and is never fabricated.

This is presentation/audit tooling, not a new scoring engine; `MarketScreenerService` and `coverage_report.py` are unaffected. `alpha_lab.research.ResearchService` is the read-only boundary between them and any UI: `ResearchService(engine, settings).get_stock_research(ticker)` reads the already-persisted current research snapshot and converts only the requested ticker through `build_stock_research` — no provider calls, no scoring, and never the whole universe merely to render one security. The Company Research Streamlit page (`app/dashboard/pages/4_Company_Research.py`) is the first consumer: it renders identity, category evidence, confidence, and strengths/weaknesses/risks entirely from `StockResearch`, and no longer inspects `LiveResearchRecord` internals (`raw_metrics`, `category_scores`, `category_coverage`, `provenance`) for that content — a handful of quote-level fields outside the canonical contract (price, market cap, Sharia status) still come from the underlying record, since they aren't research evidence. Historical/backtest research remains entirely separate and untouched.

### Historical research snapshots (`alpha_lab.research.snapshots`)

AlphaLab distinguishes **current research** (`ResearchService.get_stock_research` — whatever the live screener would show right now) from **historical research** (an immutable record of what AlphaLab's research looked like at a specific point in time). `ResearchSnapshotRepository` persists the latter in a new, append-only `research_snapshots` table (created automatically by the existing `create_schema`; nothing pre-existing is altered) — deliberately separate from `CurrentResearchBuild`/`CurrentResearchSnapshot`, which already persist an append-only history of the pre-canonical `LiveResearchRecord` for the live screener but are scoped to that purpose and only ever read back as "latest." A `research_snapshots` row stores the fully-materialized `StockResearch` JSON exactly as it was computed, so a historical snapshot can never be affected by a later change to `build_stock_research`'s logic (a formula fix, a weight change, a provider-capability update, ...) — each row is tagged with the `research_schema_version` it was written under (`alpha_lab.research.snapshots.RESEARCH_SCHEMA_VERSION`) and is never reinterpreted as a newer version.

- **Identity and deduplication**: `snapshot_id` is a deterministic SHA-256 of `{ticker, evaluation_date, rating_version, configuration_hash, payload_hash}` — not an auto-increment ID, and not keyed on `generated_at` (re-persisting identical research seconds apart returns the existing snapshot instead of creating a duplicate; a real change to the evidence or configuration always gets a new ID). `payload_hash` is a SHA-256 of the canonical `StockResearch` JSON with `generated_at` excluded, answering "did the research content actually change?", not a cryptographic guarantee over the whole database.
- **Immutability**: `ResearchSnapshotRepository` exposes only `save`/`get`/`get_latest`/`list_for_ticker`/`exists` — there is no update method, so a persisted row cannot be mutated by later current-research changes, provider corrections, or configuration changes. Concurrent identical saves race safely on the database's unique constraint on `snapshot_id`, not an application-level existence check.
- **Writing is explicit only**: `ResearchService.persist_snapshot(stock_research)` is the only write path, and it is never called by a read (`get_stock_research`, `list_current_research`, `get_research_history`, `get_latest_snapshot`). The Company Research page only writes when its "Save research snapshot" button is clicked; opening the page, switching tickers, expanding a category, viewing history, or comparing snapshots never does. A storage failure on `persist_snapshot` raises rather than being swallowed; the page surfaces it as an error rather than claiming the snapshot was saved.
- **Reading history**: `ResearchService.get_research_history(ticker)` returns lightweight `ResearchSnapshotSummary` rows (metadata only, queried by ticker at the database level) for a timeline view without deserializing every historical payload; `get_research_snapshot(snapshot_id)` / `get_latest_snapshot(ticker)` return the full, frozen `StockResearch` for one entry, never rebuilt from current data.
- **Comparison** (`alpha_lab.research.comparison.compare_stock_research`): a pure diff between two `StockResearch` objects, reporting overall score/coverage/confidence changes, per-category score/coverage/status changes, and per-metric changes explicitly typed as `MISSING_TO_VALUE`, `VALUE_TO_MISSING`, `VALUE_CHANGED`, `STATUS_CHANGED`, or `NO_CHANGE` — a category moving from no evidence to `PARTIAL` is reported as an evidence-completeness change, never implied to be a score improvement. This is historical comparison, not causal explanation: it never infers *why* something changed.

Architectural note: [Daniel Chen's `Alpha_lab`](https://github.com/Danielchen0101/Alpha_lab) was reviewed for durable run/history persistence patterns (append-only operational records, database-backed durable state behind a repository boundary, saved/read-only artifacts) — those principles are reflected above, adapted to AlphaLab's existing SQLite/SQLAlchemy architecture rather than adopting that project's technology stack (Supabase/Postgres, Flask/React, trading execution) wholesale.
