...
- Deterministic `strengths`/`weaknesses` (categories scoring ≥70/≤30 on the existing 0-100 scale) and `risks` (an unavailable category, or a data-quality issue) — never derived from an unavailable category's absence being treated as a negative.
- `catalysts` is always `[]` in this stage: it is reserved for AI Research (see below) and is never fabricated.

This is presentation/audit tooling, not a new scoring engine; `MarketScreenerService`, `coverage_report.py`, and the Streamlit pages are unaffected.
