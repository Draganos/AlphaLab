...
if st.button("Run point-in-time backtest", type="primary"):
    ...
    try:
        ...
        st.plotly_chart(px.line(curves, title="Strategy vs benchmark NAV"))
        drawdowns = curves / curves.cummax() - 1
        st.plotly_chart(px.line(drawdowns, title="Drawdown"))
        ...
        st.plotly_chart(px.line((result.cash / result.nav).rename("cash allocation")))
        st.subheader("Trades")
